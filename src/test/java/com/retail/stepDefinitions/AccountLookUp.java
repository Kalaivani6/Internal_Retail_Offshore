...
//			Thread.sleep(4000);
	...
	public void deviceDetails() throws Exception{
		try{
			...
			if((isElementDisplayed(acctDetail.Address,5)) && (isElementDisplayed(acctDetail.AccountStatus,5)))
			{
				ExtentReporter.reportStep(getDriver(), "Device Details Page is displayed", "PASS",  1);
				log.info("Device Details Page is displayed");
			}
			else
			{
				ExtentReporter.reportStep(getDriver(), "Device Details Page is not displayed", "FAIL",  0);
				log.info("Device Details Page is not displayed");
			}
